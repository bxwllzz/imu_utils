...
imu::AllanGyr::AllanGyr(std::string name, int maxCluster) :
    ...
    std::cout << m_name << " " << " num of Cluster " << numCluster << std::endl;
}
...
void imu::AllanGyr::calc() {
    std::cout << m_name << " " << " numData " << numData << std::endl;
    if (numData < 10000)
        std::cout << m_name << " " << " Too few number" << std::endl;

    double start_t = m_rawData.begin()->t;
    double end_t = m_rawData[numData - 1].t;
    std::cout << m_name << " " << " start_t " << start_t << std::endl;
    std::cout << m_name << " " << " end_t " << end_t << std::endl;
    std::cout << m_name << " " << "dt " << std::endl
            //
    ...
    if ((end_t - start_t) / 60 < 10)
        std::cout << m_name << " " << " Too short time!!!!" << std::endl;

    m_freq = getAvgFreq();
    std::cout << m_name << " " << " freq " << m_freq << std::endl;

    double period = getAvgPeriod();
    std::cout << m_name << " " << " period " << period << std::endl;
...
std::vector<double> imu::AllanGyr::getVariance() const {
    return mVariance;
}

std::vector<double> imu::AllanGyr::getDeviation() {
    double period = getAvgPeriod();
...
std::vector<double> imu::AllanGyr::getTimes() {
    double period = getAvgPeriod();
...
std::vector<int> imu::AllanGyr::getFactors() const {
    return mFactors;
}

std::vector<double> imu::AllanGyr::calcVariance(double period) {
    ...
    for (int i = 0; i < numFactors; i++) {
        ...
        for (int k = 0; k < max; k++) {
            double temp = (m_thetas[k + 2 * factor] - 2 * m_thetas[k + factor]
                    + m_thetas[k]);
            sigma2[i] += (temp * temp);
        }
...
std::vector<double> imu::AllanGyr::calcThetas(const double freq) {
...
std::vector<double> imu::AllanGyr::getLogSpace(float a, float b) {
...
double imu::AllanGyr::getFreq() const {
    return m_freq;
}
